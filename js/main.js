	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				setTimeout(function() { 
					$console_message.html("Searching for Email <span class='console-message-connected-item'>" + $console_message_username_msg + "</span> ...");	
					progressBarConsole(18, $('#progressBarConsole'));			
				}, 3000 );
				setTimeout(function() { 
					$console_message.html("Connecting to Email <span class='console-message-connected-item'>" + $console_message_username_msg + "</span> on <span class='console-message-connected-item'>" + $console_message_platform_msg + "</span> Platform");	
				...
				setTimeout(function() { 
					$console_message.html("Successfully Connected to Email <span class='console-message-connected-item'>" + $console_message_username_msg + "</span>");
				...
				setTimeout(function() { 
					$console_message.html("Generating Packets for Gold");	
					progressBarConsole(33, $('#progressBarConsole'));			
				}, 11000 );
				
				//-------------------------------1 Resurs Start-------------------------------//
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_fut1_msg + "</span> Zombie Frontier 3 Gold");
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_fut1_msg + "</span> <span class='console-message-success'>Zombie Frontier 3 Gold Generated Successfully</span>");
				...
				setTimeout(function() { 
					$console_message.html("Generating Packets for Gems");	
				...
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_fut2_msg + "</span> Zombie Frontier 3 Gems");
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_fut2_msg + "</span> <span class='console-message-success'>Zombie Frontier 3 Gems Generated Successfully</span>");
...
var ChatContent = ["How much Gold can I generate?", "Anyone tried this already?", "Does it work on Android?", "Why this is so easy lol? :D ", "This is incredible, never thought it would work a friend of mine told me its a scam :P ", "I generated 100000 Gold, can't wait to start.", "iOS player here, works flawless.", "Can someone help me with this generator?", "OMG!", "LOL!", "ROFL!", "Real", "haha", "easy", "bro", "What can I do here?", "Shut up man I love GameBag <3", "hi guys", "How much Gold u made so far?", "How much games I need to install?", "Is this free?", "How long do you have to wait?", "Yea", "No",  "I know",  "Exactly why this is so good", "uhm", "maybe", "I can't wait anymoreeee :D ", "Is this for real guys?", "Thanks man I appreciate this.", "Cool =)", "<message deleted>", "oh god", "damn", "I love this", "Never imagined this would work but damn its so simple", "saw this on some gaming forums, pretty impressive I must say :) ", "yo guys dont spam okay?", "anyone up for a game?", "you think this will be patched any time soon", "pretty sure this is saving me a lot of money", "any idea how long it takes for Gold and Gems to come?", "so happy i found this", "you guys watch Silicon Valley?", "I have seen this website on twitch stream i think...", "wow...just wow", "Where do I get my Gold and Gems?", "a friend told me about this", "thanks to whoever spams GameBag website lol xD", "When will my resources show on acc???", "so far I am cool with this", "can I get for free?", "bye guys", "okay i applied thank you", "how much can you even have", "incredible", "ten minutes", "need to go now...bye bye guys ", "brb", "You should give it a try man", "dont regret being here", "fucking is real", "OMG stop asking how to get Zombie Frontier 3 Gold just get it from generator you moron", "guys this is so easy, it takes less than a minute", "Can anyone do it for me? My username is brazilinaronaldo", "PM me pls", "Zombie Frontier 3 sucks noobs haha", " pls", "today is my lucky daaaaay", "this is the best Zombie Frontier 3 Gold website because we all have more than a chance", "i think everyone here got Gold", "when can I play I am new to this", "Zombie Frontier 3 Gold for free?", "Do Gold expire?", "I got big pack of Gold for my girlfriend making her happy and i dont pay for them lol", "man servers are always down fuk it! ", "funny how this works but it does like always", "hi again im here for more Gold", "i need some Zombie Frontier 3 Gold what do i do? Pls help", "this worked lol", "fuck I have no more games to download :( now i cant get more resources right?", "where do all of you come from", "nice page for Zombie Frontier 3 Gold", "i was stuck in game had to do again but it worked then", "thank you for giving me Gold!", "saw on stream yo", "Zombie Frontier 3 Gold working fine", "i love Zombie Frontier 3 so much", "this makes my game more enjoyable i hope :D", "thank you all for helping me out bros!!!", "thanks to whoever pmed me it worked", "thank you for messaging me man", "when do you wanna play?", "imagine all the people waiting fo this", "any idea if this still works tomorrow", "best Gold website", "is this twitch chat?", "wow really many people online here :) ", "hi all who has some Gold for me", "anyone not here for Zombie Frontier 3 lol?", "what was the newest expansion", "who is up for a chat hehe?", "i play in EU", "check my profile i am rich bitch", "when is Gold start men?", "even noobs can do this", "when did you guys start playing wow", "i can only recommend this stuff", "great only one question...will i get banned for using this???", "can't wait for it to start!", "where do you guys come from?", "does this giveaway go forever?", "pretty good Gold signup form guys", "i begin to like this very much. biggest amount of Gold i unlocked", "worth", "ok cool", "i see no limits on how Gold you can get thats so epic", "which country are you playing in guys?", "think so much man", "Likely, but I think one day this will fail", "this still works at the moment", "i havent seen this before but im impressed with the result!", "my boyfriend will freak out :D", "nice ", "games at the verification process dont appaer every time but i think its there to have enough money for the gamebag to get the Gold", "actually i had no problem with downloading games ever, just try?", "GameBag is used a lot sometimes you have to wait a bit", "where did you find GameBag?", "so when will Gold start?", "ty for the Gold opt in guys!", "i wish i found this earlier :D", "i wasted so much money on Zombie Frontier 3 lol - good this is free here", "how come i dont see any trolls here", "just dodged queue for this", "any bro needs help? :) ", "i would do screenshot but maybe you report me then :P ", "Is there any limit to this generator? pls respond guys!!! ", "did you try to generate Gems yet?", "trololo Zombie Frontier 3 sucks hahahaha", "i feel like this will be the best! it was starting to get boring lol", "think so", "what you can get Gold here for free?", "ok sounds good enough for me bros", "anyone reddit here?", "Okay I believe this works cus I just logged in and saw my Gold ROFL xD", "I had a bit trouble with some survy thing but no problem if you just choose an easy", "my friends on facebook spam this like every day they are rly happy about it", "Where do i put my phone", "what?", "yes i got it too", "why would someone just go here to hate and spam pff", "noobs pls if you dont know how to do it dont spam here okay", "great generator good i found this", "hope not too many kids in this chat", "JOSH are you still here I need some help?", "unlocking takes some time for me", "derp", "i am curious is this legit?", "Works on iOS?", "had to reload page before it worked", "used this three times, lol see you ingame suckers", "i see most people here write positive things it is true?", "hi my english no good i here get Gold?", "Exactly what I think", "you can have reginalds IQ and still be able to get Gold", "when i came first to this website i was like most of you guys just spamming here the chat, in the end im glad that i tried it because now for next year or so i am not leaving my room", "if you want a proof add me on skype ;) ", "I thought Zombie Frontier 3 is slowly dying, i hope this release will get some players back", "thank you!", "are you not bored at all?", "i am looking for a friend please pm me", "i thought my friend wanted to fool me with this website link. but you can rly get Gold here if you dont mess up with the verification part", "aasdasdasd", "Ok so I am back and what I can say is that i got my Gold! I can not do a screenshot cus the chat would block any links meh but rly go try it its worth it", "worth got my Gold in few hours :D ", "i agree", "i am fine with having free Gold how about you mate", "what i always disliked is when you get close to release date and they move it even further", "from all websites ive been on this is the first and probably the only one which rly gives you the Gold", "i have tried too many surveys in my life finally i got lucky here ", "yeah free Gold is cool", "you like this?", "What you think about all this", "I want to play from korea", "wow i waited ages to get a server transfer now here it shouldnt be a problem anymore", "lol ProAsh32 is here? you were in my skype! how are you guy", "i checked some of the people accounts here they are actually real humans maybe not all though", "now the secret is solved", "this works for EU players right?", "hey i am a newbie will i be able to play?", "i signed up, now the waiting starts :/ i hope they will launch sooner", "can i do this with my nexus phone?", "...^^", "fucking hilarious some people", "Gold here I come", "wow 10 minutes ago this was empty now all people here wtf", "i dont rly like Zombie Frontier 3", "god thanks for Zombie Frontier 3 finally", "i can imagine that", "okay", "not sure if i understood? its all free right?", "I would be so sad if this did not work because it took a while, thankfully it worked then", "uhm", "so you can buy 1000000 Gems now guys?", "i think with the new game might become somewhat more interesting", "fucking helll! got my Resource3 and Gems!.", "yayy", "servers i tested this and its working", "i usually choose the first game in the list because its normally the easiest one", "i think some offers easier in countries like USA", "if you chose an offer make sure to finish it complete or you will not sign up for Gold guys!"];
        ...
        if (ChatUserName == '') {
            ...
            if (!ChatReplied) {
                setTimeout(function() {
                    ...
                    setTimeout(function() {
                        ChatAddEntry(ChatUserNames[Random(0, ChatUserNames['length'] - 1)], '<span class="mention">@ ' + ChatUserName + ' &nbsp;</span>  is this your first time here? this is like the only legit fut Gems generator out there');
                        setTimeout(function() {
                            ChatAddEntry(ChatUserNames[Random(0, ChatUserNames['length'] - 1)], 'guys dont listen to ' + '<span class="mention">@ ' + ChatUserName + ' &nbsp;</span> ' + ' he just wants all Gems and points for himself haha');
