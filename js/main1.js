$(document).ready(function() {
	...
	var select = $( "#futGold-amount-wrapper" );
	var slider = $( "<div id='slider-futGold'></div>" ).insertAfter( select ).slider({
		min: 200000,
		max: 1000000,
		value: 200000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-futGold" ).slider( "option", "value" );				
			$('#futGold-amount').html(sliderValue);
			if(sliderValue == '200000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-futGold').addClass('btn-disabled');
				$('.max-amount-Gold').fadeOut();
			}
			else if (sliderValue == '400000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-futGold').removeClass('btn-disabled');
			}
				...
				$('#increase-futGold').removeClass('btn-disabled');
				$('.max-amount-Gold').fadeOut();
			}
			else if (sliderValue == '1000000') {
				progressBar(100, $('#progressBar'));
				$('#increase-futGold').addClass('btn-disabled');
				$('.max-amount-Gold').fadeIn();
			}
		}        
	});	
	
	$('#increase-futGold').click(function() {
	var sliderCurrentValue = $( "#slider-futGold" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 200000 );
	});

	$('#decrease-futGold').click(function() {
	var sliderCurrentValue = $( "#slider-futGold" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 200000 );
	});

	//-------------------------------
	function progressBarWood(percent, $element) {
		var progressBarWoodWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarWoodWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarWood(20, $('#progressBarWood'));
	var selectWood = $( "#futwood-amount-wrapper" );
	var sliderWood = $( "<div id='slider-futwood'></div>" ).insertAfter( selectWood ).slider({
		min: 200000,
		max: 1000000,
		value: 200000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-futwood" ).slider( "option", "value" );				
			$('#futwood-amount').html(sliderValue);
			if(sliderValue == '200000') {
				progressBarWood(20, $('#progressBarWood'));
				$('#decrease-futwood').addClass('btn-disabled');
				$('.max-amount-wood').fadeOut();
			}
			else if (sliderValue == '400000') {
				progressBarWood(40, $('#progressBarWood'));
				$('#decrease-futwood').removeClass('btn-disabled');
			}
			else if (sliderValue == '600000') {
				progressBarWood(60, $('#progressBarWood'));
			}
			else if (sliderValue == '800000') {
				progressBarWood(80, $('#progressBarWood'));
				$('#increase-futwood').removeClass('btn-disabled');
				$('.max-amount-wood').fadeOut();
			}
			else if (sliderValue == '1000000') {
				progressBarWood(100, $('#progressBarWood'));
				$('#increase-futwood').addClass('btn-disabled');
				$('.max-amount-wood').fadeIn();
			}
		}        
	});	
	
	$('#increase-futwood').click(function() {
	var sliderWoodCurrentValue = $( "#slider-futwood" ).slider( "option", "value" );
	  sliderWood.slider( "value", sliderWoodCurrentValue + 200000 );
	});

	$('#decrease-futwood').click(function() {
	var sliderWoodCurrentValue = $( "#slider-futwood" ).slider( "option", "value" );
	  sliderWood.slider( "value", sliderWoodCurrentValue - 200000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_futGold_msg = $('#slider-futGold').slider("option", "value");   
				var $console_message_futpoints_msg = $('#slider-futpoints').slider("option", "value");
				var $console_message_futwood_msg = $('#slider-futwood').slider("option", "value");
				...
				setTimeout(function() {
					...
					progressBarConsole(13, $('#progressBarConsole'));			
				}, 0 );
				setTimeout(function() { 
				...
				setTimeout(function() { 
					...
					progressBarConsole(48, $('#progressBarConsole'));			
				}, 1800 );
				setTimeout(function() { 
					$console_message.html("Searching for available server..." );	
					progressBarConsole(52, $('#progressBarConsole'));			
				}, 3000 );
				setTimeout(function() { 
					$console_message.html("Connecting to Server 1");	

					progressBarConsole(66, $('#progressBarConsole'));			
				}, 5000 );
				setTimeout(function() { 
					$console_message.html("<span class='console-message-error'>Failed to connect on Server 1</span>");
				...
				setTimeout(function() { 
					$console_message.html("Connecting to Server 2");	
					progressBarConsole(78, $('#progressBarConsole'));			
				}, 10000 );
				setTimeout(function() { 
					$console_message.html("<span class='console-message-error'>Failed to connect on Server 2</span>");	
					progressBarConsole(83, $('#progressBarConsole'));			
				}, 11000 );

				setTimeout(function() { 
					$console_message.html("Connecting to Server 3");	
					progressBarConsole(89, $('#progressBarConsole'));			
				}, 12900 );
				setTimeout(function() { 
					$console_message.html("<span class='console-message-success'>Connected to Server 3</span>");	
					progressBarConsole(94, $('#progressBarConsole'));			
				}, 16900 );
				setTimeout(function() { 
					$console_message.html("<span class='console-message-success'>Done!</span>");
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
        ...
        if (ChatUserName == '') {
            ...
            if (!ChatReplied) {
                setTimeout(function() {
                    ...
                    setTimeout(function() {
                        ChatAddEntry(ChatUserNames[Random(0, ChatUserNames['length'] - 1)], '<span class="mention">@ ' + ChatUserName + ' &nbsp;</span>  is this your first time here? this is like the only legit fut Gold generator out there');
                        setTimeout(function() {
                            ChatAddEntry(ChatUserNames[Random(0, ChatUserNames['length'] - 1)], 'guys dont listen to ' + '<span class="mention">@ ' + ChatUserName + ' &nbsp;</span> ' + ' he just wants all Gold and points for himself haha');
